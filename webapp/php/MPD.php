...
class mpd {
	...
	function Play($song) {
		if (is_numeric($song)) {
			if ($this->debugging)
				echo "mpd->Play()\n";
			if (!is_null($rpt = $this->SendCommand(MPD_CMD_PLAY, $song)))
				$this->RefreshInfo();
			if ($this->debugging)
	...
	function GetCurrentSong() {
		if ($this->debugging)
			echo "mpd->GetCurrentSong()\n";
		if (is_null($resp = $this->SendCommand(MPD_CMD_CURRENTSONG)))
			return NULL;

		$curArray = array();

		$curLine = strtok($resp, "\n");
		while ($curLine && $curLine != "OK") {
			list ( $element, $value ) = split(": ", $curLine, 2);

			$curArray[$element] = $value;
...
#   		$this->playlist = $this->_parseFileListResponse($plStr);
		$this->playlist_count = $status['playlistlength'];
